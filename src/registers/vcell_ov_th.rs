...
impl VCellOvTh {
    /// Get the Programmable cell OV fault threshold (8bit)
    #[inline]
    pub const fn get_vcell_ov_th(&self) -> u8 {
        (self.0 & 0x00FF) as u8
    }

    /// Set the Programmable cell OV fault threshold (8bit)
    #[inline]
    pub const fn set_vcell_ov_th(&mut self, vcell_ov_th: u8) {
        self.0 = self.0 & 0xFF00 | (vcell_ov_th as u16);
    }

    /// Get the Programmable cell OV event counter threshold (4 bit)
    #[inline]
    pub const fn get_ncell_ov_cnt_th(&self) -> u8 {
        ((self.0 & 0x0F00) >> 8) as u8
    }

    /// Set the Programmable cell OV event counter threshold (4 bit)
    #[inline]
